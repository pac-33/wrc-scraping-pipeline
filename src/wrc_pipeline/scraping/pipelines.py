...
2. ``PersistencePipeline`` — the single place that talks to storage, written as
   ...
   and keeps up to ``CONCURRENT_ITEMS`` items in flight, so Mongo round trips
   (PyMongo's async client) and uploads (boto3 in a worker thread) overlap with
   downloading instead of blocking the event loop. Per document: compare the
   file hash against MongoDB; unchanged documents only get their seen-markers
   touched (no upload), changed/new documents are uploaded first and upserted
   after — so a landing record always points at bytes that exist in the bucket.

Pipeline hooks take no ``spider`` argument (deprecated in Scrapy 2.13+); the
crawler handed to ``from_crawler`` provides the spider, stats and signals.
"""
...
class PersistencePipeline:
    ...
    async def open_spider(self) -> None:
        ...
        self._landing = AsyncMetadataRepository(database[self._settings.mongo.landing_collection])
        self._runs = AsyncRunReportStore(database[self._settings.mongo.runs_collection])
        ...
        await asyncio.to_thread(self._store.ensure_bucket, self._bucket)
    ...
    async def _persist_document(self, item: DocumentItem) -> DocumentItem:
        ...
        if stored_hash == item.file_hash:
            ...
            stats.inc_value("wrc/files_skipped_unchanged")
            spider.logger.debug(
                "document_unchanged",
                extra={"identifier": item.identifier, "partition": item.partition_key},
            )
        else:
            # boto3 is synchronous: run the upload in a worker thread so the
            # event loop keeps downloading while the bytes go to the bucket.
            await asyncio.to_thread(
            ...
            stats.inc_value("wrc/files_uploaded")
            ...
            stats.inc_value("wrc/records_inserted" if inserted else "wrc/records_updated")
            spider.logger.info(
                "document_stored",
                extra={
                    "identifier": item.identifier,
        ...
        stats.inc_value("wrc/records_scraped")
        return item
    ...
    async def _persist_attachment(self, item: AttachmentItem) -> AttachmentItem:
        await asyncio.to_thread(
            ...
            item.raw_body,
            content_type=item.content_type,
            metadata={"file-hash": item.file_hash, "source-url": item.url},
        )
        await self._landing.add_attachment(
            item.identifier,
            AttachmentRef(
                url=item.url,
                file_path=item.file_path,
    ...
    async def _on_spider_closed(self, spider: Spider, reason: str) -> None:
        try:
            await self._write_run_report(spider, reason)
        finally:
