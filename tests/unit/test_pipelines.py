...
class TestHashingPipeline:
    def test_html_document_fields(self) -> None:
    ...
    def test_volatile_comment_does_not_change_html_hash(self) -> None:
    ...
    def test_pdf_attachment_fields(self) -> None:
...
def process(persistence: PersistencePipeline, item) -> None:
...
class TestPersistencePipelineIdempotency:
    def test_first_run_uploads_and_upserts(
        ...
        process(persistence, make_document_item())
    ...
    def test_rerun_with_unchanged_content_skips_upload(
        ...
        process(persistence, make_document_item(HTML_V1))
        first = persistence._mongo_database[settings.mongo.landing_collection].find_one(
            {"_id": "ADJ-00053864"}
        )

        process(persistence, make_document_item(HTML_V1_REFETCH))
        ...
        assert second["last_run_id"] == spider.run_id
        assert (
    ...
    def test_changed_content_is_reuploaded(
        ...
        process(persistence, make_document_item(HTML_V1))
        process(persistence, make_document_item(HTML_V2))
    ...
    def test_attachment_persisted_and_linked(
        ...
        process(persistence, make_attachment_item())
...
class TestPersistenceConcurrency:
    def test_items_persist_concurrently_on_one_event_loop(
        ...
        """Scrapy awaits process_item for many items at once; the pipeline must
        cope with interleaved coroutines, which is what CONCURRENT_ITEMS implies."""
