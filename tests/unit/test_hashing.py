...
def test_volatile_server_comment_does_not_change_html_hash() -> None:
    """Two fetches of the same page differ only by the ASP.NET timing comment
    (verified against the live site) — the HTML hash must treat them as equal,
    even though the raw bytes differ."""
    assert file_hash(PAGE_FETCH_1) != file_hash(PAGE_FETCH_2)
    assert file_hash(PAGE_FETCH_1, is_html=True) == file_hash(PAGE_FETCH_2, is_html=True)


def test_real_content_change_is_detected() -> None:
    assert file_hash(PAGE_FETCH_1, is_html=True) != file_hash(PAGE_CHANGED, is_html=True)


def test_whitespace_reflow_does_not_change_html_hash() -> None:
    a = b"<html><body>Decision   text</body></html>"
    b = b"<html><body>Decision\n  text</body></html>"
    assert file_hash(a, is_html=True) == file_hash(b, is_html=True)


def test_binary_hash_is_of_exact_bytes() -> None:
    pdf = b"%PDF-1.4 binary bytes \x00\x01"
    assert file_hash(pdf) == file_hash(pdf, is_html=False)
    assert file_hash(pdf) != file_hash(pdf + b" ")
